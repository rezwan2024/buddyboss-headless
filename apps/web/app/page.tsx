      ...
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm text-black/70">
            ...
            <span className="text-black/40">· {timeAgo(activity.date)}</span>
          </p>
...
export default async function HomePage() {
  ...
  return (
    <main className="mx-auto w-full max-w-2xl flex-1 px-4 py-8">
      ...
      <p className="mt-1 text-sm text-black/50">{total} updates total</p>
      {items.length === 0 ? (
        <p className="mt-8 text-black/50">No activity yet.</p>
      ) : (
        <ul className="mt-6">
          {items.map((activity) => (
