...
export { ACCESS_TOKEN_COOKIE, REFRESH_TOKEN_COOKIE, USER_COOKIE };
...
export async function setSessionCookies(tokens: TokenResponse): Promise<void> {
  const store = await cookies();
  store.set(ACCESS_TOKEN_COOKIE, tokens.access_token, accessTokenCookieOptions(tokens.expires_in));
...
export async function clearSessionCookies(): Promise<void> {
  const store = await cookies();
  store.set(ACCESS_TOKEN_COOKIE, "", expiredCookieOptions(true));
  store.set(REFRESH_TOKEN_COOKIE, "", expiredCookieOptions(true));
  store.set(USER_COOKIE, "", expiredCookieOptions(false));
}
