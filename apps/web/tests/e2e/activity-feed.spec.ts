...
test("activity feed renders real data with no console errors @smoke", async ({ page }) => {
  ...
  // Scoped to <main>, not the whole page — the activity home page also has
  // sidebar cards with their own <li> items outside <main>.
...
test("scrolling loads more activity @smoke", async ({ page }) => {
  ...
  const initialCount = await page.locator("main li").count();
  await page.locator("main li").last().scrollIntoViewIfNeeded();

  await expect
    .poll(async () => page.locator("main li").count(), { timeout: 5000 })
